...
$(document).ready(function () {
    ...
    function changeEnergy(value) {
        energy.style.backgroundPositionX = "0";
        if (value === 1.00) {
            energy.style.backgroundPositionY = "0";
        } else if (value === 0.00) {
            energy.style.backgroundPositionY = "-148px";
        } else {
            energy.style.backgroundPositionY = (parseInt(value * 100 / 20) - 4) * 37 + "px";
        }
        console.log("energy.style.backgroundPositionY" + " ---- " + energy.style.backgroundPositionY)
        $.ajax({  
            type : "GET",  // 提交方式
            url : "electricity/"+deviceId,// 路径
    ...
    var bglists = document.querySelectorAll('.speedbg li');
    ...
    for (var i = 0; i < speedList.length; i++) {
        ...
        speedList[i].addEventListener('touchstart', function (e) {
        	e.preventDefault();
            for (var j = 0; j < bglists.length; j++) {
                bglists[j].className = 'hide';
            };
            bglists[this.index].className = 'show';
            switch (this.index) {
                case 0:
                    multiple = 50;
                    Addnum(50);
