...
init.startBtn.click(function () {
	if (playMode === 'music') {
		sendOrder.sendMusic('startBtn');
	} else {
		sendOrder.sendStory('startBtn');
	}
	control.start();
	changeVoice();
	autoplay();
...
init.last.click(function () {
	...
	autoplay();
	if (playMode === 'music') {
		sendOrder.sendMusic('last');
	} else {
		sendOrder.sendStory('last');
	}
});
...
init.mode.click(function () {
	control.loop();
	if (playMode === 'music') {
		sendOrder.sendMusic('loop');
	} else {
		sendOrder.sendStory('loop');
	}
});
init.slideBtn.click(function () {
	if (playMode === 'music') {
		sendOrder.sendMusic('sport');
	} else {
		sendOrder.sendStory('sport');
	}
	control.sport();
});
...
function switchBtn(ele, status) {
	if (ele === 'startBtn') {
		if (status === 0) {
			init.startBtn.css({ 'backgroundPosition': -init.startBtnW + 'px 0' });
		}
		if (status === 1) {
			init.startBtn.css({ 'backgroundPosition': '0 0' });
		}
	}
	if (ele === 'sport') {
		if (status === 1) {
	...
	if (ele === 'loop') {
		if (status === 0) {
			init.mode.css({ 'backgroundPosition': '0 0' });
		} else {
			init.mode.css({ 'backgroundPosition': -init.modeW + 'px 0' });
		}
	}
};
